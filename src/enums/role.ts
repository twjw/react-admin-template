...
>(data: () => T, labelKey = 'label' as LK): EnumInstance<T, LK, FTK> {
	...
	function getByKeyVal(this: FTK, val: Eval, key: FTK | LK) {
		if (cacheMap == null) newCache()
		const label = cacheMap!.get(val)!
		return result[label][key] || label
	}

	function getByLabel(val: keyof T, key: FTK) {
		if (cacheMap == null) newCache()
		return result[val][key] || val
	}

	for (let k in _data) {
		for (const k2 in _data[k]) {
			;(result[`getBy${k2[0].toUpperCase()}${k2.substring(1)}`] as any) = getByKeyVal.bind(
				k2 as unknown as FTK,
			)
		}
		break
	}
...
const e11 = a.getByLabel('USER', 'value')
const e12 = a.getByLabel('USER', 'level')
...
console.log(a.keys)
console.log(a)
console.log({ e1, e11, e12, e2, e22, e3 })
